...
function mergeFiles(dataFile,geoJsonFile, fullOutputFile){
    ...
    for (var entry in geoJsonFile.features){
        temp = geoJsonFile.features[entry].properties.NAME10
        temp = temp.split(' County Precinct ')
        county = temp[0]
        for (var i in defaultDiacriticsRemovalMap){
            county = county.replace(defaultDiacriticsRemovalMap[i].letters, defaultDiacriticsRemovalMap[i].base) 
        }
        precinct = geoJsonFile.features[entry].properties.prec_num
        district = geoJsonFile.features[entry].properties.uscong_dis
        results = dataFile.filter(function(entry){
        ...
        geoJsonFile.features[entry].properties.entryId = entry
        if (!results || results.length == 0){
            console.log('No results')
            results = dataFile.filter(function(entry){
                if (entry.county.toLowerCase() == county.toLowerCase() ){
                    // console.log(entry);
                }
                return entry.county.toLowerCase() == county.toLowerCase() && parseInt(precinct) == entry.precinct
            });
            ...
            console.log(geoJsonFile.features[entry].properties)
        }
        total = results.map(function(entry){
            return entry.votes
        }).reduce(function(e1,e2){
            return e1 + e2
        },0)
        dem = results.filter(function(entry){
            return entry.party == 'Democratic'
        })
        rep = results.filter(function(entry){
            return entry.party == 'Republican'
        })
        if (rep.length > 0) rep = rep[0]
        else rep = 0;
        geoJsonFile.features[entry].properties.rep = rep
        if (dem.length > 0) dem = dem[0]
        else dem = 0;
        geoJsonFile.features[entry].properties.dem = dem
    ...
    for (var i = 0; i < geoJsonFile.features.length; i++){
        for (var j = parseInt(i)+1; j< geoJsonFile.features.length; j++){
            ...
            if (intersection){
                if (!geoJsonFile.features[i].properties.neighbours){
                    geoJsonFile.features[i].properties.neighbours = []
                }
                geoJsonFile.features[i].properties.neighbours.push(j)
                if (!geoJsonFile.features[j].properties.neighbours){
                    geoJsonFile.features[j].properties.neighbours = []
                }
